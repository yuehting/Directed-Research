...
with open("all_opportunities_df.pkl", 'rb') as infile:
    df = pkl.load(infile)
...
if os.path.isfile("all_opportunities_df_cleaned.csv") == True:
    old_dataframe = pd.read_csv("all_opportunities_df_cleaned.csv")
    old_dataframe = old_dataframe.drop(['Unnamed: 0'], axis=1)
    result = pd.concat([old_dataframe, df], ignore_index=True, sort=False)
    print(result)
    result.drop(columns=result.columns[-1], axis=1, inplace=True)
    print(result)
    result.to_csv("all_opportunities_df_cleaned.csv")
else:
    df.to_csv("all_opportunities_df_cleaned.csv")
#print(old)
