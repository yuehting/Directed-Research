...
def make_df(all_opportunities, columns):
    ...
    program_collection = []
    for o in range(
            len(all_opportunities)):  # o is the all_opportunities-loop, going from 0 to 842 (843 entries, as of April 1st 2021)
            ...
            if df.columns[d] != "program":
                if df.columns[d] in all_opportunities[o]:
                    if type(all_opportunities[o][df.columns[d]]) == list:  # strip the strings
                        df.loc[o, df.columns[d]] = [el.strip() if type(el) == str else el for el in
                                                    all_opportunities[o][df.columns[d]]]
                    else:
