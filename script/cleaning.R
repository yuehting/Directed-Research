...
source_python('~/Directed_Research_R/scraping.py')
source_python('~/Directed_Research_R/preprocessing.py')

setwd("~/Directed_Research_R")
opps_total <- read_csv("data/all_opportunities_df_cleaned.csv")
summary(opps_total)
opps_total <- select(opps_total, -c(...1))
summary(opps_total)
# remove duplicated items
opps_total <- distinct(opps_total)
summary(opps_total)

# transform datatype from character to categorical
opps_total$cost <- as.factor(opps_total$cost)
summary(opps_total$cost)
opps_total$locationCity <- as.factor(opps_total$locationCity)
opps_total$locationState <- as.factor(opps_total$locationState)
opps_total$typeOfOpportunity <- as.factor(opps_total$typeOfOpportunity)
summary(opps_total$locationState)
