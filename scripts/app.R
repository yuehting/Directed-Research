...
opps_inspect$locationState <- as.factor(opps_inspect$locationState)
opps_inspect$locationCity <- as.factor(opps_inspect$locationCity)
opps_inspect$cost <- as.factor(opps_inspect$cost)
summary(opps_inspect$cost)
# to see each state launches how many projects
...
opps_inspect <- opps_inspect %>%
  mutate(StartDate = '',
         EndDate = '')

opps_inspect$StartDate <- as.Date(opps_inspect$fromDate)
opps_inspect$EndDate <- as.Date(opps_inspect$toDate)
...
ui <- fluidPage(
    ...
    sidebarLayout(
        sidebarPanel(
          ...
          p("The data utilized here was scrapied from:"),
...
server <- function(input, output) {
  ...
  output$PieChart <- renderPlot(width = 1000,
    ...
    sum <- sum(new$value)
    new <- new %>%
      mutate(percentage = round(value/ sum,2)) %>%
      filter(new$value!=0) 
